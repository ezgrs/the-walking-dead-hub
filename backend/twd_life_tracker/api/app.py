...
import twd_life_tracker.api.fastapi_lifespan_dependencies
import twd_life_tracker.api.dependencies.settings
import twd_life_tracker.api.dependencies.database_engine
import twd_life_tracker.api.routers.entities
import twd_life_tracker.api.routers.seasons
...
def create_app() -> fastapi.FastAPI:
    ...
    lifespan = twd_life_tracker.api.fastapi_lifespan_dependencies.Lifespan()
    lifespan.register(twd_life_tracker.api.dependencies.settings.evaluate)
    lifespan.register(
        twd_life_tracker.api.dependencies.database_engine.evaluate
    )

    app = fastapi.FastAPI(
        title="The Walking Dead Hub",
        description=(
            "The Walking Dead character database providing a complete overview of each character's journey."
        ),
    ...
    app.include_router(
        twd_life_tracker.api.routers.entities.router, prefix="/entities"
    )
    app.include_router(
        twd_life_tracker.api.routers.seasons.router, prefix="/seasons"
    )
